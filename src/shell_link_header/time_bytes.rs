    ...
    fn default() -> Self {
        match SystemTime::now().duration_since(SystemTime::UNIX_EPOCH) {
            Ok(n) => {
                ...
                Self(unix_to_ldap_timestamp(n.as_secs() as i64, 0))
            }
            // SystemTime before UNIX EPOCH!
            Err(_) => {
                println!("n.seconds: ERROR");
                Self(unix_to_ldap_timestamp(0, 0))
            }
        }
    }
}
...
impl Filetime {
    ...
    pub fn new_from_unix(unix_seconds: i64, unix_nanoseconds: u64) -> Self {
        Self(unix_to_ldap_timestamp(unix_seconds, unix_nanoseconds))
    }
